...
try {
  ...
  const captureMailer = createCaptureMailer();
  const auth = authStore(db, authConfig, { mailer: captureMailer, webOrigin: 'http://127.0.0.1:5173' });
  ...
  stage = 'login lockout and history';
  ...
  for (let attempt = 0; attempt < 4; attempt += 1) await assert.rejects(() => auth.login({ email, password: 'wrong password' }), InvalidCredentials);
  await assert.rejects(() => auth.login({ email, password }), AccountUnavailable);
  const locked = await db.query('SELECT "failedLoginCount", "lockedUntil" FROM "User" WHERE "id" = $1', [authUserId]);
  assert.equal(locked.rows[0].failedLoginCount, 5); assert.ok(locked.rows[0].lockedUntil);
  await db.query('UPDATE "User" SET "lockedUntil" = NOW() - INTERVAL \'1 second\' WHERE "id" = $1', [authUserId]);
  const login = await auth.login({ email: email.toUpperCase(), password }, { ipAddress: '10.0.0.1', userAgent: 'Known browser' });
  await auth.login({ email, password }, { ipAddress: '10.0.0.2', userAgent: 'New browser' });
  assert.equal(Number((await db.query('SELECT COUNT(*) AS count FROM "SecurityEvent" WHERE "userId" = $1 AND "type" = \'NEW_DEVICE_LOGIN\'', [authUserId])).rows[0].count), 1);
  assert.ok(Number((await db.query('SELECT COUNT(*) AS count FROM "LoginHistory" WHERE "userId" = $1', [authUserId])).rows[0].count) >= 8);
  stage = 'new-device detection';
  const loginIdentity = await verifyAccessToken(authConfig, login.accessToken);
  assert.equal(loginIdentity.userId, authUserId);
  stage = 'status and inactivity enforcement';
  ...
  await db.query('UPDATE "User" SET "status" = \'SUSPENDED\' WHERE "id" = $1', [authUserId]);
  await assert.rejects(() => auth.validateAccess(authUserId, loginIdentity.sessionId), InvalidCredentials);
  await db.query('UPDATE "User" SET "status" = \'ACTIVE\' WHERE "id" = $1', [authUserId]);
  await db.query('UPDATE "RefreshTokenFamily" SET "lastSeenAt" = NOW() - INTERVAL \'2 hours\' WHERE "id" = $1', [loginIdentity.sessionId]);
  await assert.rejects(() => auth.validateAccess(authUserId, loginIdentity.sessionId), InvalidCredentials);
  await db.query('UPDATE "RefreshTokenFamily" SET "lastSeenAt" = NOW() WHERE "id" = $1', [loginIdentity.sessionId]);
  ...
  assert.equal(captureMailer.messages.length, 1);
  const verificationSecret = new URL(captureMailer.messages[0].text).searchParams.get('token');
  assert.ok(verificationSecret);
  const storedVerification = await db.query('SELECT "tokenHash", "expiresAt", "consumedAt" FROM "EmailVerificationToken" WHERE "userId" = $1 ORDER BY "createdAt" DESC LIMIT 1', [authUserId]);
  assert.match(storedVerification.rows[0].tokenHash, /^[a-f0-9]{64}$/); assert.notEqual(storedVerification.rows[0].tokenHash, verificationSecret);
  await auth.consumeEmailVerification(verificationSecret);
  await assert.rejects(() => auth.consumeEmailVerification(verificationSecret), InvalidVerificationToken);
  ...
  const detail = safeDetail && error instanceof Error ? ` ${error.message}` : '';
