...
| [x] | T0004 | Add the Vite entry HTML and React root. | Completed: apps/web/index.html, src/main.tsx, src/App.tsx and workspace-local tsconfig.json mount the ComplyOS placeholder. Verified workspace Vite production build, TypeScript --noEmit and browser-rendered heading/text at http://127.0.0.1:5173/. Theme/component styling remains T0005. | T0003 |
| [x] | T0005 | Copy the existing Tailwind theme and Shadcn configuration into apps/web. | Completed: copied theme CSS, Button and cn utility unchanged; adapted Tailwind content paths/ESM, Shadcn client/CSS settings, PostCSS and Vite/TypeScript aliases. Installed explicit workspace dependencies. Production build and TypeScript check pass; browser verified light/dark button colors, 40px height and 6px radius. Build reports a non-blocking outdated Browserslist dataset warning. | T0004 |
| [x] | T0006 | Add React Router v6 and an empty authenticated shell route. | Completed: React Router 6.30.6, shared AppShell/Outlet, /dashboard placeholder, root redirect and not-found recovery link. Build and TypeScript check pass; browser verified direct URL, refresh, unknown route, recovery link and root redirect. This is the future authenticated layout only; actual session enforcement belongs to the identity tasks and no tenant data is loaded. | T0005 |
| [x] | T0007 | Port the sidebar component and replace Next Link usage. | Completed: copied sidebar with React Router Link/useLocation, segment-safe active matching and aria-current; mounted in AppShell with responsive spacing. All 15 destinations use shared navigation definitions; unfinished modules show explicit placeholders. Build and TypeScript checks pass. Browser checked section collapse/expand, mobile open/selection-close, Dashboard/Vendors/Settings navigation and active state. Broader batch check timed out; no claim of full end-to-end module validation. | T0006 |
| [x] | T0008 | Port the topbar and replace usePathname. | Completed: topbar uses React Router useLocation; visible title/subtitle and browser title follow the route. Mounted in AppShell with one sidebar offset and mobile menu clearance. Unimplemented search/help/notification actions are disabled and the fake unread dot is removed. Build and TypeScript checks pass; browser verified Programs → Evidence title updates and reload. | T0007 |
| [x] | T0009 | Port the existing shared badges and score ring. | Completed: StatusBadge, PriorityBadge, SeverityBadge and ScoreRing ported to apps/web/src/components/shared/badges.tsx with exported prop interfaces. Preserved original variants/score thresholds and added accessible ring labels. Dashboard previews are explicitly sample data. Build and TypeScript checks pass; browser/screenshot verified badges and 45/65/85 percent rings. | T0008 |
